...
static Boolean iscounting(const char *name) {
	int c;
	const char *s = name;
	while (*s == '0') /* mwgamera: list assertion fix */
		s++;
	if (*s == '\0') /* must have at least one non-zero digit */
		return FALSE;
	while ((c = *s++) != '\0')
		if (!isdigit(c))
			return FALSE;
...
extern void validatevar(const char *var) {
	if (*var == '\0')
		fail(SHNAME":var", "zero-length variable name");
	if (iscounting(var))
		fail(SHNAME":var", "illegal variable name: %S", var);
#if !PROTECT_ENV
	if (strchr(var, '=') != NULL)
		fail(SHNAME":var", "'=' in variable name: %S", var);
#endif
}
