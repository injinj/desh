...
cmd	:	    %prec LET   { $$ = NULL; }
	...
	| '!' optcaret cmd	{ $$ = prefix("%not",
                                       mk(nList, thunkify($3), NULL)); }
...
assign	: optcaret '=' optcaret words	{ $$ = $4; }

fn	: FN word params '{' body '}'	{ $$ = fnassign($2, mklambda($3, $5)); }
	| FN word		{ $$ = fnassign($2, NULL); }

first	: comword		{ $$ = $1; }
	| first nlcaret sword	{ $$ = mk(nConcat, $1, $3); }

sword	: comword		{ $$ = $1; }
	| keyword		{ $$ = mk(nWord, $1); }

word	: sword			{ $$ = $1; }
	| word nlcaret sword	{ $$ = mk(nConcat, $1, $3); }
